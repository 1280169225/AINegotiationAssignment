...
public class BRAMAgent extends Agent {
	/* FINAL VARIABLES */
    ...
    private  ArrayList<Bid> opponentBidsArray;//An Array that contains the last 100 bids that the opponent agent offered
		...
		try{
			...
			for (int i=0; i<10; i++) 
			{
				...
				if (currBid == null){
					...
					double currUtility =  utilitySpace.getUtilityWithDiscount(currBid, timeline);
	...
	private Bid getBidFromBidsArray()
	{
		...
		if(ourBidsArray.size() == 1)
		{
			...
			fillBidsArray(timeline.getElapsedSeconds());
	...
	private void fillBidsArray(double startTime){
		...
		int countNewBids = 0;
		while(timeline.getElapsedSeconds() - startTime < TIME_TO_CREATE_BIDS_ARRAY && countNewBids < bidsMaxAmount){
			try {
