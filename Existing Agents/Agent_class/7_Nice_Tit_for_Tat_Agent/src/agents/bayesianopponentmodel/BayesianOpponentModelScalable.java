...
/*					} else {
						lWeightHyps.get(j).get(i).setProbability(fWeightHyps.get(j).get(i).getProbability()/lN);
					}*/				
				}
			}		
	//	}
...
//		for(int k=0;k<5;k++){
		...
		for(int i=0;i<fEvaluatorHyps.size();i++) {
			...
			for(int j=0;j<fEvaluatorHyps.get(i).size();j++) {
				EvaluatorHypothesis lHyp = new EvaluatorHypothesis(fEvaluatorHyps.get(i).get(j).getEvaluator());
