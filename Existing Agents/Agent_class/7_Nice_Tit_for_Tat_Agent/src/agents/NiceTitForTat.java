...
public class NiceTitForTat extends BilateralAgent
{
	...
	private void updateMyNashUtility()
	{
		...
		try
		{
			...
			log("Nash estimate: (" + nash.utilityA + ", " + nash.utilityB +"), so I aim for " + percentage(nashMultiplier) + "%: " + round2(myNashUtility));
		} catch (Exception e)
		{
	...
	public Bid chooseOpeningBid()
	{
		...
		} catch (Exception e)
		{
			// TODO Auto-generated catch block
			e.printStackTrace();
			return utilitySpace.getDomain().getRandomBid();
		}
	}
...
//		System.out.println(s);
	...
	private static Bid getRandom(List<Bid> bids)
	{
		int range = bids.size();
