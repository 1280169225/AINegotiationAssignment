...
public class CUHKAgent extends Agent {
    ...
    public Action chooseAction() {
        ...
        try {
            ...
            } else if (ActionOfOpponent instanceof Offer) {//the opponent propose first and we response secondly
                //update opponent model first
                this.opponentBidHistory.updateOpponentModel(((Offer) ActionOfOpponent).getBid(), utilitySpace.getDomain(), this.utilitySpace);
        ...
        try {
            ...
            minimumOfBid = this.MaximumUtility - decreasingAmount_4;
                ...
                if (timeline.getTime() <= this.concedeToDiscountingFactor) {
                    ...
                    this.utilitythreshold = maximumOfBid - (maximumOfBid - minThreshold) * Math.pow((timeline.getTime() / this.concedeToDiscountingFactor), alpha1);
                } else {
                    this.utilitythreshold = (maximumOfBid * this.discountingFactor) / Math.pow(this.discountingFactor, timeline.getTime());
                }
            ...
            if (utilitySpace.getUtility(bestBidOfferedByOpponent) >= this.utilitythreshold || utilitySpace.getUtility(bestBidOfferedByOpponent) >= minimumOfBid) {
    ...
    private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
        ...
        if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
            ...
            if (currentMaximumUtility > predictMaximumUtility && timeline.getTime() > this.concedeToDiscountingFactor) {
                try {
    ...
    private int estimateRoundLeft(boolean opponent) {
        ...
        round = (this.totalTime - timeline.getCurrentTime()) / (this.maximumTimeOfOpponent + this.maximumTimeOfOwn);
    ...
    private void chooseConcedeToDiscountingDegree() {
       ...
       /* if (this.maximumOfBid > 100) {
        ...
        System.out.println("concedeToDiscountingFactor is " + this.concedeToDiscountingFactor + "current time is " + timeline.getTime());
    }
    /*
